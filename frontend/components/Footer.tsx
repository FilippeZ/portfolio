...
import { Github, Linkedin, Instagram, ArrowUp, ShieldCheck } from "lucide-react";
            ...
            <div className="max-w-7xl mx-auto px-6 relative z-10">
                    ...
                    <div className="lg:col-span-6 space-y-8">
                        <div>
                            ...
                            <p className="text-gray-400 text-base lg:text-lg font-light leading-relaxed max-w-md">
                                {t.tagline_prefix} <span className="text-gray-300 font-medium">{t.tagline_highlight}</span>
                            </p>
                        </div>
                        ...
                        <div className="flex gap-4 pt-4">
                            {[
                                ...
                                { icon: Linkedin, href: "https://www.linkedin.com/in/filippos-paraskevas-zygouris-8a88a8266/", label: "LinkedIn" },
                                { icon: Instagram, href: "https://www.instagram.com/philip_zygouris/", label: "Instagram" }
                            ].map((social, i) => (
                                <a
                                    key={i}
                                    href={social.href}
                        ...
                        <div className="p-6 rounded-2xl bg-white/[0.02] border border-white/5 relative group hover:bg-white/[0.04] hover:border-white/10 transition-all duration-500 overflow-hidden min-w-[240px]">
                            ...
                            <div className="relative z-10 flex flex-col gap-5">
                                <div className="flex items-center gap-3 bg-black/40 backdrop-blur-md rounded-full px-4 py-2 w-max border border-white/5 shadow-[0_0_15px_rgba(34,197,94,0.1)] group-hover:shadow-[0_0_20px_rgba(34,197,94,0.2)] transition-shadow">
                                    ...
                                    <span className="text-gray-200 text-xs font-bold tracking-wide">{t.all_systems_operational}</span>
                                </div>

                                <div className="space-y-1">
                                    <div className="flex justify-between items-center text-xs text-gray-500 font-mono">
                                        <span>{t.build_version}</span>
                                        <span className="text-blue-400 font-medium">{t.version_number}</span>
                                    </div>
                                    <div className="h-px w-full bg-white/5 my-2"></div>
                                    <div className="flex justify-between items-center text-xs text-gray-500 font-mono">
                                        <span>{t.last_build}</span>
                                        <span className="text-gray-400">{t.last_build_date}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div className="pt-8 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-6 relative">
                    <p className="text-gray-500 text-xs md:text-sm font-medium order-2 md:order-1 flex items-center gap-2">
                        &copy; 2026 <span className="text-gray-300">Filippos Zygouris.</span> {t.rights_reserved}
                    </p>
